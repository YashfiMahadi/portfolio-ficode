...
export function ProfileHeaderCard({ profile, uploading, onUploadPhoto }: ProfileHeaderCardProps) {
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      ...
      <div className="px-6 pb-6">
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400">
          ...
          {profile.github && (
            <a href={profile.github} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
              🐙 GitHub
            </a>
          )}
          {profile.linkedIn && (
            <a href={profile.linkedIn} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
              💼 LinkedIn
            </a>
          )}
          {profile.website && (
        ...
        {profile.tentangSaya && (
          <p className="mt-4 text-sm leading-relaxed text-gray-600 dark:text-gray-400">{profile.tentangSaya}</p>
        )}
      </div>
    </div>
  );
