...
import { jenjangColor } from "../constants";
...
export function EducationList({ loading, data, onEdit, onDelete }: EducationListProps) {
  return (
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex gap-4">
                ...
                <div>
                    ...
                    {item.jenjang && (
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                          jenjangColor[item.jenjang] || "bg-gray-600 text-gray-50"
                        }`}
                      >
                        {item.jenjang}
                      </span>
                  ...
                  {item.deskripsi && (
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{item.deskripsi}</p>
                  )}
                </div>
              </div>
